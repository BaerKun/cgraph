...
// return a <= x < b
static inline int32_t randint32(const int32_t a, const int32_t b) {
  return a + pcg32_random() % (b - a);
}

static inline int64_t randint64(const int64_t a, const int64_t b) {
  return a + ((uint64_t)pcg32_random() << 32 | pcg32_random()) % (b - a);
}

static inline float randfp32(const float a, const float b) {
  return a + (b - a) * (float)pcg32_random() / 4294967296.f;
}

static inline double randfp64(const double a, const double b) {
